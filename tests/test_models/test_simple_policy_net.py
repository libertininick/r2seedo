...
def test_psp_raises_on_invalid_depth() -> None:
...
@pytest.mark.parametrize("num_sub_policies", [1, 2, 3, 4])
@pytest.mark.parametrize("act_dim", [2, 3, 4])
@pytest.mark.parametrize("input_dim", [(8,), (3, 8), (4, 1), (2, 12, 8)])
def test_psp_output_shape(
    input_dim: tuple[int, ...], act_dim: int, num_sub_policies: int
) -> None:
    """Test output shape of ParallelSubPolicies."""
    *other_dims, obs_dim = input_dim
    if not other_dims:
        other_dims = [1]

    psp = ParallelSubPolicies(
        obs_dim=obs_dim,
        act_dim=act_dim,
        hidden_dim=16,
    ...
    assert y.shape == (*other_dims, num_sub_policies, act_dim)
